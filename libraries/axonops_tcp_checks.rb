    ...
    action :create do
      converge_by("Creating/updating AxonOps TCP check #{new_resource.name}") do
        begin
          ...
          Chef::Log.info("Starting AxonOps TCP check processing for: #{new_resource.name}")
          client = AxonOps.new(
          ...
          Chef::Log.info("Found existing TCP check: #{old_check ? 'YES' : 'NO'}")
          Chef::Log.info("Existing TCP check data: #{old_check}") if old_check
          ...
          if changed || old_check.nil?
            if new_resource.present
              ...
              Chef::Log.info("TCP check payload: #{tcp_check_payload}")
