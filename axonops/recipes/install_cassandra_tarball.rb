...
# Get configuration
cassandra_version = node['cassandra']['version']
...
directory node['cassandra']['directories']['logs'] do
  owner cassandra_user
  group cassandra_group
  mode '0755'
  recursive true
...
directory node['cassandra']['directories']['gc_logs'] do
  owner cassandra_user
  group cassandra_group
  mode '0755'
  recursive true
...
execute 'set-vm-max-map-count' do
  ...
  not_if 'test $(sysctl -n vm.max_map_count) -ge 1048575'
end

file '/etc/sysctl.d/99-cassandra.conf' do
  content 'vm.max_map_count=1048575'
  mode '0644'
end
